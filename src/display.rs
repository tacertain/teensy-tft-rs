    ...
    DC: OutputPin,
{
    ...
    pub fn init(&mut self) -> Result<(), DisplayError> {
        ...
        self.write_command(0x01)
            .map_err(|_| DisplayError::InitializationFailed)?; // SWRESET
        delay.delay_ms(120);

        // Exit sleep mode
        self.write_command(0x11)
            .map_err(|_| DisplayError::InitializationFailed)?; // SLPOUT
        delay.delay_ms(120);

        // Display configuration
        self.write_command(0x3A)
            .map_err(|_| DisplayError::InitializationFailed)?; // COLMOD - Pixel Format Set
        self.write_data(&[0x55])
            .map_err(|_| DisplayError::InitializationFailed)?; // 16-bit RGB565

        // Memory Access Control
        self.write_command(0x36)
            .map_err(|_| DisplayError::InitializationFailed)?; // MADCTL
        self.write_data(&[0x48])
            .map_err(|_| DisplayError::InitializationFailed)?; // MY=0, MX=1, MV=0, ML=0, BGR=1, MH=0

        // Display on
        self.write_command(0x29)
            .map_err(|_| DisplayError::InitializationFailed)?; // DISPON
        delay.delay_ms(100);
    ...
    pub fn fill_rect(
        ...
        let color_bytes = [(color_value >> 8) as u8, (color_value & 0xFF) as u8];
...
pub struct DoubleBufferedDisplay<'a, SPI, DC, P, const N: u8> {
...
impl<'a, SPI, DC, P, const N: u8> DoubleBufferedDisplay<'a, SPI, DC, P, N>
where
    SPI: Write<u8>,
    DC: OutputPin,
{
    ...
    pub fn present(&mut self) -> Result<(), DisplayError> {
        ...
        self.display
            .set_address_window(0, 0, self.width - 1, self.height - 1)
            .map_err(|_| DisplayError::SpiError)?;

        self.dma.set_disable_on_completion(true);

        let front_buffer_u32: &[u32] = unsafe {
            slice::from_raw_parts(
                self.front_buffer.as_ptr() as *const u32,
                self.front_buffer.len() / 2,
            )
        };
...
impl<'a, SPI, DC, P, const N: u8> DrawTarget for DoubleBufferedDisplay<'a, SPI, DC, P, N>
where
    SPI: Write<u8>,
    DC: OutputPin,
{
...
impl<'a, SPI, DC, P, const N: u8> OriginDimensions for DoubleBufferedDisplay<'a, SPI, DC, P, N>
{
    fn size(&self) -> Size {
        Size::new(self.width as u32, self.height as u32)
    }
